        ...
        <ul class="v-comment_list">
            <li class="v-comment_list-item" v-for="comment in data">
                <div class="v-comment_head">
                    <div class="v-comment_avatar" :style="getAvatar(comment)">
                        {{comment.avatar ? '' : comment.nick_name.substr(0, 1)}}
                    </div>
                ...
                <div class="v-comment_foot">
                    ...
                    <v-comment-form enable-cancel v-if="reply && comment.id==reply.id" @cancel="reply=null"
                                    @submit="submitComment" :placeholder="placeholder" :enable-submit="enableSubmit">
                        <template slot="form-foot">
                            <slot name="form-foot"></slot>
                        </template>
                    </v-comment-form>
